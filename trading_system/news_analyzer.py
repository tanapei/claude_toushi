...
以下の形式で、LINE通知用に簡潔（合計700文字以内）にまとめてください：

■ 本日の注目ニュース（最大3件・各1行）
・<ニュース概要>

■ 注目買い候補（上昇期待トップ3）
1. <銘柄名>（<ティッカー>）— <期待できる理由を1行で>
2. <銘柄名>（<ティッカー>）— <期待できる理由を1行で>
3. <銘柄名>（<ティッカー>）— <期待できる理由を1行で>
...
注意：
- 「注目買い候補」は対象銘柄リストから選び、ニュースに基づいた根拠がある場合のみ挙げてください
- 根拠が薄い場合は「本日は特になし」としてください
- 投資判断はあくまで参考情報です
"""
